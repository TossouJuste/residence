                        ...
                        <div class="col-lg-12" data-aos="fade-up" data-aos-delay="100">
                            ...
                            <table class="table table-bordered w-100 table-striped">
                                <tbody>
                                    <tr>
                                        <td>Nom & Prénoms</td>
                                        <td>{{ $demande->etudiant->nom ?? 'Non disponible' }} {{ $demande->etudiant->prenom ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td>Année d'étude et filière</td>
                                        <td>{{ $demande->annee_etude ?? 'Non disponible'}}<sup>e</sup> année {{ $demande->filiere ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td>École</td>
                                ...
                                <table class="table table-bordered w-100">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <ul>
                                                    <li>Première vague : 2500F x 9 mois + caution 5000F = <strong>27 500F</strong></li>
                                                    <li>Deuxième vague : 2500F x 7 mois + caution 5000F = <strong>22 500F</strong></li>
                                                </ul>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td colspan="2">
                                                @if (!$classement->paiement)
                                                    <button class="btn-success" onclick="afficherFormulaire()">Payer les frais de résidence</button>

                                                    <div id="formulairePaiement" class="hidden mt-3">
                                                        <form action="{{ route('paiement.make', $classement->id) }}" method="POST">
                                                            @csrf
                                                            <table>
                                                                <td>
                                                                    <div class="mb-3">
                                                                    <label for="telephone">Téléphone :</label>
                                                                    <input type="tel" name="telephone" required class="form-control" placeholder="Ex : 97000000" id="telephone">
                                                                </div>
                                                                </td>
                                                                <td>
                                                                <div class="mb-3">
                                                                    <label for="email">Email :</label>
                                                                    <input type="email" name="email" required class="form-control" placeholder="Ex : test@example.com" id="email">
                                                                </div>
                                                                </td>
                                                            </table>
                                                            <button type="submit" class="btn-success">Payer</button>
                                                        </form>
                                                    </div>
                                                @else
