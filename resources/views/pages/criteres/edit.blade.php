            ...
            <div class="card-body">
                <form action="{{ route('cabines.update', $cabine->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="nbr_place" class="form-control" value="{{ old('nbr_place', $cabine->nbr_place) }}" required>
                        @error('nbr_place')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        <label for="places_en_etat" class="form-label">Places en État</label>
                        <input type="number" name="places_en_etat" class="form-control" value="{{ old('places_en_etat', $cabine->places_en_etat) }}" required>
                        @error('places_en_etat')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
